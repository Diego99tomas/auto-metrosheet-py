...
def fill_excel_data(layout_excel:dict,wb, function_name:str, ranges_list:list[str], mid_index:int|None,frecuencias:list[str]|list[None]):
    ...
    mid_val, mid_unit = parse(ranges_list[mid_index]) if mid_index is not None else (None, None)
    ...
    for item in ranges_list:
        ...
        #Consulta a lyout si es un mulktimetro 
        elif layout.get('is_mult') and val >1 and layout.get('special_unit')==unit:
            target_row_start = current_row
            puntos = layout['special_point']
        ...
        for p in puntos:
            ...
            for freq in frecuencias:
                ...
                #Todos los puntos de pinzas usan la bobina por lo tanto (punto/50)
                if layout.get('is_pinza'):
                    punto_buscado=punto_buscado/50
                    if layout.get('lcomp'):
                        lcomp=1     

                decimal, show_espc = specs_search_in_db(function_name, punto_buscado, unit,freq,lcomp)
                # ESCRITURA EN EXCEL
