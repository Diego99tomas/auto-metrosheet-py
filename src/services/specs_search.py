...
def specs_search_in_db(type_table:str,value:float,unid:str, frec:str|None=None, lcomp:int|None=None):
    ...
    if is_ac:
        if lcomp is None:
            query= f"SELECT decimales,show_spec FROM {type_table} WHERE valor=? AND frecuencia=? AND lcomp is NULL"
            params=(busqueda,frec)
        else:    
            query= f"SELECT decimales,show_spec FROM {type_table} WHERE valor=? AND frecuencia=? AND lcomp=?"
            params=(busqueda,frec,lcomp)
